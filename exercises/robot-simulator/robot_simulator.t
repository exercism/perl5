...
is_deeply $robot->coordinates, [ 3, 0 ], "set robot coordinates"
  or diag explain $robot->coordinates;

$robot->at( -2, 5 );
is_deeply $robot->coordinates, [ -2, 5 ], "set robot coordinates with negative x"
  or diag explain $robot->coordinates;

$robot->at( 0, 0 );
$robot->orient($north);
$robot->advance;
is_deeply $robot->coordinates, [ 0, 1 ], "add 1 to y coordinate after robot advances from north"
  or diag explain $robot->coordinates;

$robot->at( 0, 0 );
$robot->orient($east);
$robot->advance;
is_deeply $robot->coordinates, [ 1, 0 ], "add 1 to x coordinate after robot advances from east"
  or diag explain $robot->coordinates;
...
is_deeply $robot->coordinates, [ 0, -1 ],
  "subtract 1 to y coordinate after robot advances from south"
  or diag explain $robot->coordinates;
...
is_deeply $robot->coordinates, [ -1, 0 ],
  "subtract 1 to x coordinate after robot advances from west"
  or diag explain $robot->coordinates;
...
is_deeply $simulator->instructions("L"), ["turn_left"], "translate L to 'turn_left'"
  or diag explain $simulator->instructions;

is_deeply $simulator->instructions("R"), ["turn_right"], "translate R to 'turn_right'"
  or diag explain $simulator->instructions;

is_deeply $simulator->instructions("A"), ["advance"], "translate A to 'advance'"
  or diag explain $simulator->instructions;

is_deeply $simulator->instructions("RAAL"), [qw(turn_right advance advance turn_left)],
  "multiple instruction translations"
  or diag explain $simulator->instructions;

$robot = $module->new;
$simulator->place( $robot, { x => -2, y => 1, direction => $east } );
$simulator->evaluate( $robot, "RLAALAL" );
is_deeply $robot->coordinates, [ 0, 2 ], "test coordinates after instructing robot"
  or diag explain $robot->coordinates;
...
is_deeply $robot_1->coordinates, [ -4, 1 ], "test coordinates after instructing robot_1"
  or diag explain $robot_1->coordinates;
is $robot_1->bearing, $west, "test bearing after instructing robot_1";

is_deeply $robot_2->coordinates, [ -3, -8 ], "test coordinates after instructing robot_2"
  or diag explain $robot_2->coordinates;
is $robot_2->bearing, $south, "test bearing after instructing robot_2";

is_deeply $robot_3->coordinates, [ 11, 5 ], "test coordinates after instructing robot_3"
