...
is_deeply $robot->coordinates, [ -2, 5 ],
  "set robot coordinates with negative x"
  or diag explain $robot->coordinates;
...
is_deeply $robot->coordinates, [ 0, 1 ],
  "add 1 to y coordinate after robot advances from north"
  or diag explain $robot->coordinates;
...
is_deeply $robot->coordinates, [ 1, 0 ],
  "add 1 to x coordinate after robot advances from east"
  or diag explain $robot->coordinates;
...
can_ok $sim_module, "new"
  or BAIL_OUT "missing sub ${sim_module}::new()";

can_ok $sim_module, "place"
  or BAIL_OUT "missing sub ${sim_module}::place()";

can_ok $sim_module, "evaluate"
  or BAIL_OUT "missing sub ${sim_module}::evaluate()";

can_ok $sim_module, "instructions"
  or BAIL_OUT "missing sub ${sim_module}::instructions()";

my $simulator = $sim_module->new;

is_deeply $simulator->instructions("L"), ["turn_left"],
  "translate L to 'turn_left'"
  or diag explain $simulator->instructions;

is_deeply $simulator->instructions("R"), ["turn_right"],
  "translate R to 'turn_right'"
  or diag explain $simulator->instructions;

is_deeply $simulator->instructions("A"), ["advance"],
  "translate A to 'advance'"
  or diag explain $simulator->instructions;

is_deeply $simulator->instructions("RAAL"),
  [qw(turn_right advance advance turn_left)],
...
is_deeply $robot->coordinates, [ 0, 2 ],
  "test coordinates after instructing robot"
  or diag explain $robot->coordinates;
...
is_deeply $robot_1->coordinates, [ -4, 1 ],
  "test coordinates after instructing robot_1"
