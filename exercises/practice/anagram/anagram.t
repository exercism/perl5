...
is( match_anagrams( "diaper", [ "hello", "world", "zombies", "pants" ] ),
    bag { end; },
    "no matches",
);

is( match_anagrams( "solemn", [ "lemons", "cherry", "melons" ] ),
    bag { item "lemons"; item "melons"; end; },
    "detects two anagrams",
);

is( match_anagrams( "good", [ "dog", "goody" ] ),
    bag { end; },
    "does not detect anagram subsets",
);

is( match_anagrams( "listen", [ "enlists", "google", "inlets", "banana" ] ),
    bag { item "inlets"; end; },
    "detects anagram",
);
...
is( match_anagrams( "go", ["go Go GO"] ),
    bag { end; },
    "does not detect an anagram if the original word is repeated",
);

is( match_anagrams( "tapper", ["patter"] ),
    bag { end; },
    "anagrams must use all letters exactly once",
);

is( match_anagrams( "BANANA", ["BANANA"] ),
    bag { end; },
...
is( match_anagrams( "LISTEN", [ "LISTEN", "Silent" ] ),
    bag { item "Silent"; end; },
