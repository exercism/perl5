...
use constant BASE_DIR => path(__FILE__)->realpath->parent->parent->parent;
...
sub new {
  ...
  if ( $self->{exercise} && !$self->{data}{ignore_cdata} ) {
    my $cdata_file = BASE_DIR->child(
      'problem-specifications/exercises/' . $self->{exercise} . '/canonical-data.json' );
...
sub _render {
  ...
  my $rendered = Template::Mustache->render(
    BASE_DIR->child( 'templates/' . $params->{template} . '.mustache' )->slurp, $data );
  Perl::Tidy::perltidy( source => \$rendered, argv => '', destination => \my $tidied );
  return $tidied;
}
